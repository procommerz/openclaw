...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    description: [
      ...
      'Profiles: use profile="chrome" unless the user explicitly asks for a different profile.',
      'If the user mentions the Chrome extension / Browser Relay / toolbar button / “attach tab”, ALWAYS use profile="chrome" (do not ask which profile).',
      'When a node-hosted browser proxy is available, the tool may auto-route to it. Pin a node with node=<id|name> or target="node".',
      "Chrome extension relay needs an attached tab: user must click the OpenClaw Browser Relay toolbar icon on the tab (badge ON). If no tab is connected, ask them to attach it. If you cannot connect to any tabs - abort your task and inform the user with error message details.",
    ...
    execute: async (_toolCallId, args) => {
      ...
      const profile = "chrome"; // Force chrome profile
